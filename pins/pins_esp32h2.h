...
/*
 * Pin definitions for ESP32-H2 (RISC-V)
 *
 * ESP32-H2 specifications:
 * - Single-core RISC-V 32-bit @ 96MHz
 * - 320KB SRAM
 * - 12-bit SAR ADC (5 channels)
 * - No built-in DAC (use PWM or I2S)
 * - BLE 5.2 + Zigbee 3.0 + Thread (NO WiFi)
 * - 27 GPIO pins
 *
 * Note: ESP32-H2 has no WiFi, so WiFi UDP mode is not available
 */

#if !defined(PINS_ESP32H2_H)
#define PINS_ESP32H2_H
...
// DAC output (TX) - ESP32-H2 has no built-in DAC, use PWM or I2S
#define PIN_TX          GPIO_NUM_3     // GPIO3 for PWM DAC
#define PIN_TX_CH       3              // DAC channel (PWM)

// Serial repeater pins (optional)
#define PIN_SERIAL2_RX  GPIO_NUM_20
#define PIN_SERIAL2_TX  GPIO_NUM_21
...
#define PIN_P25         GPIO_NUM_22    // P25 mode LED
#define PIN_NXDN        GPIO_NUM_23    // NXDN mode LED
#define PIN_POCSAG      GPIO_NUM_24    // POCSAG mode LED
#define PIN_M17         GPIO_NUM_25    // M17 mode LED
#define PIN_FM          GPIO_NUM_2     // FM mode LED
#endif

// ESP32-H2 specific settings
#define ESP32H2_ADC_BITS    12         // 12-bit ADC resolution
#define ESP32H2_ADC_SAMPLES 1          // No hardware averaging

// Buffer sizes (smaller due to limited RAM and lower CPU speed)
#define ESP32H2_RX_BUFFER_SIZE  256
#define ESP32H2_TX_BUFFER_SIZE  256

// Timer settings
#define SAMPLE_RATE         24000      // 24kHz sample rate
#define TIMER_DIVIDER       96         // 96MHz CPU / 96 = 1MHz timer clock
#define TIMER_ALARM_VALUE   (1000000 / SAMPLE_RATE)  // ~41.67 for 24kHz

// PWM DAC settings (when USE_PWM_DAC is defined)
// ESP32-H2 only has LOW_SPEED_MODE (no HIGH_SPEED_MODE)
#define PWM_FREQUENCY       78125      // 78.125kHz PWM carrier
#define PWM_RESOLUTION      12         // 12-bit resolution (0-4095)
#define PWM_CHANNEL         LEDC_CHANNEL_0
#define PWM_SPEED_MODE      LEDC_LOW_SPEED_MODE  // Only LOW_SPEED on H2
